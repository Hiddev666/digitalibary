...
class Peminjaman_model
{
    ...
    public function cancelPending($userid, $bukuid)
    {
        $this->stmt = $this->dbh->prepare("DELETE FROM peminjaman WHERE id_user=:userid AND id_buku=:bukuid AND status_peminjaman='Pending'");
        $this->stmt->execute([
            "userid" => $userid,
            "bukuid" => $bukuid
        ]);
    ...
    public function cancelPinjam($userid, $bukuid)
    {
        $this->stmt = $this->dbh->prepare("DELETE FROM peminjaman WHERE id_user=:userid AND id_buku=:bukuid AND status_peminjaman='Dipinjam'");
        $this->stmt->execute([
            "userid" => $userid,
            "bukuid" => $bukuid
        ]);
